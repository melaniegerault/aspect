...
public:
    ...
    Point<dim> representative_point(const double depth) const
    {
        ...
        if (dim==2)
            return Point<dim>(1.5, 1.0-depth);
        else
    ...
    get_used_boundary_indicators () const
    {
        ...
        ids.insert(0);ids.insert(1);ids.insert(2);ids.insert(3);
        return ids;
    }
    ...
    get_symbolic_boundary_names_map () const
    {
        ...
        names["wedge-top"] = 0;
        names["wedge-bottom"] = 1;
        names["top"] = 2;
        names["sides and bottom"] = 3;
        return names;
    }
...
ASPECT_REGISTER_GEOMETRY_MODEL(Wedge,
                               "wedge",
                               "Wedge geometry for benchmarking a suduction-zone.")
